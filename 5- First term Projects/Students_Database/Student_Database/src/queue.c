...
void add_student_from_txt(void){
	...
	FILE *file;
	char path[] = "C:\\Users\\Mega Store\\Documents\\GitHub\\Embedded-Practice\\5- First term Projects\\Students_Database\\eclipse\\Student_Database\\src\\Students.txt";
	...
	/*Read contents of the file using any of these functions:
	 *  fgetc()  : get character
	 *  fgets(), : get string line by line
	 *  fscanf() : get get formated/structured text
	 *  fread(): get the whole text within the file */

	while(fgets(studentsFile, 150, file ) != NULL){
		printf("Students from the file: %s\n", studentsFile);

		/* Use sscanf to split the string based on commas */
...
void show_all(void){
	...
	if (length == 0 ){
		printf("Not valid operation!, List is empty\n");
		return;
	}
	for (int i = 0; i < length; i++){
		printf("\nFirst Name: %s \n", (queue + i)->fristName);
